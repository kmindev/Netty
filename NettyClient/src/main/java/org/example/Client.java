...
public class Client {
    ...
    public void connect() throws InterruptedException {
        ...
        // Bootstrap 생성 및 설정
        Bootstrap bootstrap = new io.netty.bootstrap.Bootstrap().group(eventLoopGroup);

        bootstrap.channel(NioSocketChannel.class);
        bootstrap.remoteAddress(new InetSocketAddress(host, port)); // 접속할 서버 소켓 주소와 포트
        bootstrap.handler(new ClientInitializer());

        // 서버 소켓에 연결하고, sync()로 기다린다.
